...
def doAtExit():
    ...
    np.save('dataT{}'.format(af),values_save_H)
    np.save('dataF{}'.format(af),values_save_P)
    print("save succesfully { ",af," }")
    print("Temperatura Guardada: ",values_save_T)
    print("Humedad Guardada: ",values_save_H)
    print("Presion Guardada: ",values_save_P)
    print("Flujo Guardada: ",values_save_F)
    print("serialArduino.isOpen() = " + str(serialArduino.isOpen()))

def plotValues():
    
    gs= gridspec.GridSpec(4,4)  # cantidad de columnas y filas
                                #4 sensores
    ...
    ax2.plot(Presion_values, 'go-', label='Presion atmosferica')
    ax2.legend(loc='upper right')

    ax2=plt.subplot(gs[3,:])#cuarto # humedad
    ax2.plot(Humedad_values, 'co-', label='Humedad')
    ax2.legend(loc='upper right')
...
def Flujo(allsensor,values_save_F,Flujo_values): #primer dato entregado
    ...
    print("Flujo Capatado: ",valuefloatF)        
...
def Temperatura(allsensor,values_save_T,Temp_values): #segundo dato en ser entregado
    ...
    scale=1
    valuefloatT= round(float(allsensor[1])*(scale))
    Temp_values.append(valuefloatT)
    print("Temperatura Capatada: ",valuefloatT)
...
def Humedad(allsensor,values_save_H,Humedad_values): #tercero dato entregado
    scale=1
    valuefloatH= round(float(allsensor[2])*(scale))
    Humedad_values.append(valuefloatH)
    print("Humedad Capatada: ",valuefloatH)        
    values_save_H=np.insert(values_save_H,values_save_H.size,float(valuefloatH))       
    Humedad_values.pop(0)
    return values_save_H
    #values.pop(0) verificar si esto sirve
    #drawnow(plotValuesH)# funcion solo para graficar los datos de temperatura h    

def Presion(allsensor,values_save_P,Presion_values): #cuarto dato entregado
    scale=1
    valuefloatP= round(float(allsensor[3])*(scale))
    Presion_values.append(valuefloatP)
    print("Presion Capatada: ",valuefloatP)        
    values_save_P=np.insert(values_save_P,values_save_P.size,float(valuefloatP))       
    Presion_values.pop(0)
    #values.pop(0) verificar si esto sirve
    #drawnow(plotValuesP)# funcion solo para graficar los datos de temperatura h
    return values_save_P
...
for i in range(0,11):#tamaño del grafico a mostrar
    ...
    Presion_values.append(0)
    Humedad_values.append(0)    
    ######        
while True:
    while (serialArduino.inWaiting()==0):
        pass
    ...
    try:
        ...
        values_save_P=Presion(allsensor,values_save_P,Presion_values)       
        values_save_H=Humedad(allsensor,values_save_H,Humedad_values)
        drawnow(plotValues)
